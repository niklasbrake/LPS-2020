...
function [header ImageData] = getTimeSeries(Folder)
  ...
  load(fullfile(Folder,[ExperimentName ' RoiMask.mat']));
  ...
  for Slice = 1:StepCount
  	...
  	I = imread(ImageName);
    points = RoiMask(Slice).points;
    coords = RoiMask(Slice).coords;    
    ImageData(Slice) = struct('Slice', Slice, 'Results', [], 'NumOfROIs',size(points,1),'Average',I,'RoiCoordinates',coords,'RoiMask',[]);
    ImageData(Slice).RoiMask = points;
  end


  % Measures average pixel value for each ROI
  Results = measure(fullfile(Folder,'Image_0001_0001.raw'), header, ImageData,[]);
...
function header = analyseTimeSeries(header, ImageData)
  ...
  h = waitbar(1/length(RoiData), 'Please Wait...', 'Name','dF/F');
