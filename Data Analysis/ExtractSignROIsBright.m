function ExtractSignROIsBright(R2)
...
save(fullfile(['SignROIsBright' header.FileName(10:end)]),'Significant_ROIs','ZScores','Allresponses');


% Write the results to Excel.

xlswrite(['SignificantResponsesAvg_Bright.xlsx'],Significant_ROIs,[(F(i).name)]);
xlswrite(['SignificantResponsesZ_Bright.xlsx'],ZScores,[(F(i).name)]);
xlswrite(['SignificantResponsesAll_Bright.xlsx'],Allresponses,[(F(i).name)]);
end 
